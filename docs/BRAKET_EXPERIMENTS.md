...
each) were submitted and are queued on the device; the next device
availability window is Friday (2026-09-18), and the tasks execute then.
Results are added by `collect` and rendered in `REPORT.md`; raw counts are
stored in the ledger. As stated above, this is an analog sampling
exploration on chain-structured instances, not an exact Ising embedding.
